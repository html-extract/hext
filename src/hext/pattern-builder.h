...
public:
  pattern_builder();
  ~pattern_builder();
  std::unique_ptr<match_pattern> build_match_and_reset();
  std::unique_ptr<capture_pattern> build_capture_and_reset();
  void reset();

  void set_builtin_function(bi::builtin_func_ptr func);
