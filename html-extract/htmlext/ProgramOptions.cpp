  ...
  vm_()
{
  ...
  this->desc_.add_options()
    ...
    ("keep-incomplete,k", "Do not remove incomplete results")
    ("no-auto-any-descendant,n", "Do not force top rules to be any descendant")
    ("cap-attrib-optional,c", "Ignore capture attributes when matching")
    ("no-merge-results,m", "Do not interleave results from top level rules")
